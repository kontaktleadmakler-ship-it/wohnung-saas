...
class MatchingTests(unittest.TestCase):
    ...
    def test_hard_filters(self):
        ...
        self.assertIsNone(match_listing(listing,self.profile))

    def test_unknown_values_are_not_rejected(self):
        listing={"url":"https://x.test/1","title":"Wohnung Berlin Mitte","description":""}
        reasons=match_listing(listing,self.profile)
        self.assertIsNotNone(reasons)
        self.assertTrue(any("nicht angegeben" in x for x in reasons))
    ...
    def test_exclusion_word_boundary(self):
        ...
        self.assertIsNotNone(match_listing(listing,self.profile))

    def test_float_values_are_used_as_numbers(self):
        listing={"url":"https://x.test/float","title":"Wohnung Berlin Mitte","description":"3 Zimmer","price_total":830.0,"rooms":3.0,"size":65.0}
        reasons=match_listing(listing,self.profile)
        self.assertIsNotNone(reasons)
    ...
    def test_german_number_strings_are_supported(self):
        listing={"url":"https://x.test/string","title":"Wohnung Berlin Mitte","description":"2 Zimmer","price_total":"830,00","rooms":"2","size":"50,0"}
        self.assertIsNotNone(match_listing(listing,self.profile))

    def test_no_ranking_score_is_returned(self):
        listing={"url":"https://x.test/1","title":"Wohnung Berlin Mitte","price_total":830,"rooms":2,"size":50}
        result=match_listing(listing,self.profile)
        self.assertIsInstance(result, list)
        self.assertFalse(any("/100" in x for x in result))
